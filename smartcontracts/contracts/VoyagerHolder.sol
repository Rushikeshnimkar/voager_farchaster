...
error VoyagerHolder__ProposalRejected();
error VoyagerHolder_ClaimedNotPossible();
error VoyagerHolder_NotEnoughFunds();

contract VoyagerHolder is Context, ERC721A  {
    ...
    modifier onlyOperator() {
        require(
            ...
            "VoyagerHolder: User is not authorized"
        );
        _;
    }
    
    modifier onlyProposalCreator {
        require(_msgSender() == proposalCreator,"VoyagerHolder: User is not proposal creator");
        _;      
    }

    modifier onlyWhenProposalIsNotActive {
        require(
            block.timestamp < fundingActiveTime,"VoyagerHolder: Funding has been intiated , action cannot be performed"
        );
        _;
    }
    modifier onlyWhenNotPaused {
        require(
            pause == false ,"VoyagerHolder: Funding is paused!"
        );
        _;
    }
    ...
    constructor(
        ...
        require(proposalDetails.length == 4,"VoyagerHolder: Invalid Proposal Input");
        ...
        require(contractAddr.length == 2,"VoyagerHolder: Invalid Contract Input");
    ...
    function mintTicket(uint256 quantity)external returns (uint256 prevQuntity,uint256 currentQuantity) {
        ...
        require(
            ...
            "VoyagerHolder: Amount cannot be minted!"
        );
        require(block.timestamp >= fundingActiveTime && block.timestamp < fundingEndTime,"VoyagerHolder: Funding cannot be done"); 
        /// if the creator didn't staked the proposal will be rejected and users can claimback
        if(isCreatorStaked == false){
                _proposalRejection();
                revert VoyagerHolder__ProposalRejected();        
        }
    ...
    function withdrawFunds(address wallet, uint256 amount) external onlyProposalCreator onlyWhenNotPaused {
        require(
            amount <= stakingAmount && fundsInReserve > 0,
            "VoyagerHolder: Amount to be collected more than staked"
        );
         require(
            fundsInReserve >= amount,
            "VoyagerCollab: Process cannot proceed , more than reserve fund"
        );
    ...
    function claimback() external nonReentrant returns(uint256 ,bool) {
       ...
       uint256 refundValue;
       require(nftBalance > 0,"VoyagerHolder: User cannot claimback");
       require(refundStatus[_msgSender()] == false,"VoyagerHolder: Refund is already claimed!");
       if(fundingEndTime < block.timestamp && fundsInReserve != crowdFundingGoal){   
           ...
           return (refundValue,refundStatus[_msgSender()]);
            ...
            revert VoyagerHolder_ClaimedNotPossible();
       }
    }   
    ...
    function yieldSubmission() external onlyProposalCreator{
        require(isYieldReturned == false,"VoyagerHolder: Action cannot be proceeded");
        ...
        if(balanceOfCreator >=  yeildToBeRecieved){
            ...
            if(deficit <= stakingAmount){
                ...
                revert VoyagerHolder_NotEnoughFunds();
            }
       }
    ...
    function unStake() external onlyProposalCreator  returns(uint256) {
        require(
            ...
            "VoyagerHolder: User cannot withdraw funds");
        require(stakingAmount > 0,"VoyagerHolder: Not Enough Staking Funds");
        if(fundsInReserve == 0){
            require(isYieldReturned == true,"VoyagerHolder: Yield is not submitted");
        }
        isCreatorStaked = false;
    ...
    function tokenURI(
        ...
        require(_exists(tokenId), "VoyagerHolder: Non-Existent Asset");
        return baseURI;
    }
